...
const COURSE_ROUTES = {
  ...
  'chatgpt-mastery': 'chatgpt-mastery.html?lesson=1',
...
async function verifyPaystackReturn(user) {
  ...
  if (paymentState !== 'success' || !reference || !courseId) return false;
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.access_token) throw new Error('Your session has expired. Please sign in again.');
  ...
  const { data: course, error: courseError } = await supabase.from('courses').select('id,slug').eq('id', courseId).maybeSingle();
  if (courseError || !course) throw new Error('Paid course was verified, but its learning route could not be resolved.');
  const entry = courseEntry(course);
  if (!entry) throw new Error('Paid course was verified, but its learning route is unavailable.');
...
async function loadCourses(user) {
  const { data: courses = [], error: coursesError } = await supabase
    .from('courses')
    .select('id,slug,title,description,level,duration_hours,lesson_count,image_path,is_published,is_free')
  ...
  grid.innerHTML = orderedCourses.map((c, index) => {
    ...
    const lessonLabel = isFree ? '<strong>FREE</strong> · ' : '';
    const priceMarkup = price && !isFree ? `<br><strong>₦${Number(price.amount).toLocaleString('en-NG')} ${escapeHtml(price.currency)}</strong>` : '';
    const actionLabel = enrolled.has(c.id) ? 'Continue Learning →' : (isFree ? 'Start Learning →' : 'Enroll & Start Learning →');
    return `<article class="course-card" data-course-id="${c.id}" data-course-slug="${escapeHtml(c.slug)}">
      ...
      <button class="btn academy-course-action" data-course-id="${c.id}" data-course-slug="${escapeHtml(c.slug)}" data-course-route="${escapeHtml(route || '')}" data-course-free="${isFree ? 'true' : 'false'}">${actionLabel}</button>
    </article>`;
  }).join('');
...
async function enrollOrContinue(button, user) {
  ...
  const isFree = button.dataset.courseFree === 'true';
  ...
  try {
    if (!isFree) {
      if (await hasActiveEntitlement(user.id, courseId)) {
        if (!route) throw new Error('This course has no configured learning route.');
