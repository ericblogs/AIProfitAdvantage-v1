...
const renderAuthor = (profile) => { const name = String(profile?.full_name || '').trim() || 'Community member'; const safeName = escapeHtml(name); const avatar = safeAvatarUrl(profile?.avatar_url); return `<div class="forum-author" aria-label="Posted by ${safeName}">${avatar ? `<img class="forum-author-avatar" src="${escapeHtml(avatar)}" alt="${safeName} avatar" loading="lazy" referrerpolicy="no-referrer" onerror="this.hidden=true;this.nextElementSibling.hidden=false">` : ''}<span class="forum-author-avatar forum-author-avatar--initials"${avatar ? ' hidden' : ''} aria-hidden="true">${initials(name)}</span><span class="forum-author-name">${safeName}</span></div>`; };
...
async function hydrateAuthors() {
  ...
  if (heading && !root.querySelector('.forum-topic-author')) heading.insertAdjacentHTML('afterend', `<div class="forum-topic-author forum-topic-author--detail">${renderAuthor(profileMap.get(topic.user_id))}</div>`);
  const postMeta = root.querySelectorAll('.forum-post-meta');
  posts.forEach((post, index) => { const meta = postMeta[index]; if (!meta || meta.dataset.authorHydrated === 'true') return; const time = meta.querySelector('time'); meta.innerHTML = `${renderAuthor(profileMap.get(post.user_id))}<span aria-hidden="true">·</span>`; if (time) meta.appendChild(time); meta.dataset.authorHydrated = 'true'; });
  root.dataset.authorsHydrated = 'true';
}

const observer = new MutationObserver(() => { if (root?.querySelector('h1')) hydrateAuthors(); });
