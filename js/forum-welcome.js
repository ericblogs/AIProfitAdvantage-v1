...
const isWelcomeTopic = root && new URLSearchParams(window.location.search).get('id') === WELCOME_TOPIC_ID;
  ...
  function openComposer() {
    ...
    if (!composer) {
      ...
      composer.querySelector('#forum-intro-submit').addEventListener('click', async () => {
        ...
        try {
          const { error } = await supabase.from('forum_posts').insert({ topic_id: WELCOME_TOPIC_ID, user_id: user.id, parent_post_id: postId, body, status: 'visible' });
